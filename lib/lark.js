...
const MESSAGE_TYPES = {
  ...
  IM_WS_DOMAINS: Symbol("IM_WS_DOMAINS"),
};
...
const TEMPLATE_COLORS = {
  ...
  [MESSAGE_TYPES.IM_WS_DOMAINS]: "orange",
};
...
function detectMessageType(msg) {
  ...
  } else if (msg.includes("IM WS Domain test")) {
    return MESSAGE_TYPES.IM_WS_DOMAINS;
  }
...
function parse(msg) {
  ...
  if (msg.includes("REPORTER") && msg.includes("FARM")) {
    ...
    messageType === MESSAGE_TYPES.PROD_WPS_TEST ||
    messageType === MESSAGE_TYPES.APP_DOMAINS ||
    messageType === MESSAGE_TYPES.IM_WS_DOMAINS
  ) {
    const lines = msg.split("\n");
    const result = [];
    lines.forEach((line, index) => {

...
import { formatDateTime, getFarm, getStatus } from "./lark_utils.js";
...
export function getMessagePost(messageText) {
  ...
  const status = getStatus(messageText);
  const emoji = ["STABLE", "SUCCESS"].includes((status || "").toUpperCase()) ? "✅" : "❌";
...
export async function sendMessageByEmail(email, messageText) {
  try {
    const response = await axios.post(
      ...
      {
        content: JSON.stringify(getMessagePost(messageText)),
        msg_type: "post",
        receive_id: email,
      },
      {
